...
G.enemy.spawn = {
  ...
  create(type, x, y, waveNumber, difficultyMult) {
    if (type === 'goblin') return new G.enemy.Goblin(x, y, difficultyMult);
    if (type === 'archer') return new G.enemy.Archer(x, y, difficultyMult);
    if (type === 'boss') return new G.enemy.Boss(x, y, waveNumber);
    return new G.enemy.Goblin(x, y, difficultyMult);
  }
};
